...
export const useCompanyController = (userId) => { 
  ...
  // fetch all the attribute

  const fetchAttributes = useCallback(async () => {
    setLoading(true);
    try {
      const data = await companyModel.getAllAttributes();
      const sorted = data
        .filter(attr => attr.bactive)
        .sort((a, b) => a.cattribute_name.localeCompare(b.cattribute_name));
  ...
  const fetchUserAttributes = useCallback(async (id) => {
      setLoading(true);
      try {
        const data = await companyModel.getUserAttributes(id); 
...
  // Fetch USER attributes when userId is available/changes
  useEffect(() => {
    if (userId) {
...
// // fetch user attributes by userId
// const fetchUserAttributes = async (userId) => {
//   setLoading(true);
//   try {
//     const data = await companyModel.getUserAttributes(userId); 
//     setUserAttributes(data);
//     setError(null);
//   } catch (err) {
//     console.error("Failed to fetch user attributes:", err);
