...
const { leadStatus, fetchLeadStatus } = useLeadStatusController();
...
const companies = [...new Set(leadStatus.map(status => status.company.cCompany_name))]; //Changes to companies

// Filter the data based on the dropdown menu else return the whole lead status
const filteredLeadStatus = selectedCompany ? leadStatus.filter(status => status.company.cCompany_name === selectedCompany) : 
leadStatus;
...
<table className="min-w-full border border-gray-300 rounded-xl overflow-hidden shadow-sm">
  ...
  <tbody className="text-sm text-gray-700">
    {filteredLeadStatus.map((status, index) => (
      <tr
        key={index}
        ...
        <td className="border px-4 py-2 font-semibold text-gray-500">{status.clead_name}</td>
        <td className="border px-4 py-2 font-semibold text-gray-500">
          <span className="inline-block px-3 py-1 rounded-full bg-indigo-100 text-indigo-700 text-xs font-semibold ">
            {status.company.cCompany_name}
          </span>
        </td>
        <td className="border px-4 py-2 font-semibold text-gray-500">{status.imodified_by || "None"}</td>
        <td className="border px-4 py-2 font-semibold text-gray-500">  {formatDate(status.dcreated_dt)}</td>
      </tr>
    ))}
  </tbody>
</table>    
