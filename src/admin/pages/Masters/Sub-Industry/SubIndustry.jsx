...
// import LeadStatusForm from './Sub-Components/';
 // Assuming this hook provides loading/error states
import formatDate from '../../../utils/formatDate';
import { useSubIndustry } from './useSubIndustry';

const SubIndustry = ({company={}} ) => {
  // Custom hooks for CRUD operations
  const { subIndustry, fetchSubIndustry, loading, error } = useSubIndustry();
  ...
  const companies = useMemo(() => {
    if (!subIndustry || subIndustry.length === 0) return [];
    return [...new Set(subIndustry.map(status => status.company?.cCompany_name || "Unknown Company"))];
  }, [subIndustry]);

  // Filter lead statuses based on selected company
  const filteredSubIndustry = useMemo(() => {
    if (!subIndustry) return []; // Handle null/undefined leadStatus
    return selectedCompany
      ? subIndustry.filter(status => (status.company?.cCompany_name || "Unknown Company") === selectedCompany)
      : subIndustry;
  }, [subIndustry, selectedCompany]);

  // Pagination Logic
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = filteredSubIndustry.slice(indexOfFirstItem, indexOfLastItem);
  const totalPages = Math.ceil(filteredSubIndustry.length / itemsPerPage);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [selectedCompany, filteredSubIndustry.length]);
